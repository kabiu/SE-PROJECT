...
$pdf->Cell(34,5,'PRODUCT',1,0,'L');//190
...
while($item=$select->fetch(PDO::FETCH_OBJ)){
	...
	$length=strlen($item->product_name);
	...
	$pdf->vcell(34,$c_height,$x_axis,$item->product_name);
	$x_axis=$pdf->getx();// now get current pdf x axis value
	$pdf->vcell(11,$c_height,$x_axis,$item->qty);
	$x_axis=$pdf->getx();
	$pdf->vcell(8,$c_height,$x_axis,$item->price);
	$x_axis=$pdf->getx();
	$pdf->vcell(12,$c_height,$x_axis,$item->price*$item->qty);
	$pdf->Ln();
	$x_axis=$pdf->getx();
}
