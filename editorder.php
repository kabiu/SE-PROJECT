...
if(isset($_POST['btnupdateorder'])){
    ...
    // write update query from tbl_product stock
    ...
    //delete query from tbl_invoice details table data wher e invoice_id =$id.
 ...
 for($i=0 ; $i<count($arr_productid) ; $i++){
  ...
  //select query from tbl_product table to get out of stock value   
...
// $db_stock[$i]=$rowpdt->pstock;    
    $db_stock[$i] = getProductQuantity($arr_productid[$i],$_SESSION['location_id'],$pdo);
   ...
   $rem_qty = $db_stock[$i]-$arr_qty[$i];
     ...
     if( $rem_qty<0){
         ...
         //update query from tbl_product table to update stock values
         
         updateInventory($arr_productid[$i], $location_id['location_id'], $rem_qty ,$pdo);
                               ...
                               <th>Enter Quantity</th>
                                 <th>Total</th>
                                        ...
                                        <th>
                                            ...
                                            <center><button type="button" name="add" class="btn btn-success btn-info btnadd" role="button"><span class="glyphicon glyphicon-plus" ></span></button></center>
...
?> 
    ...
    $stock_db=getProductQuantity($item_invoice_details['product_id'],$_SESSION['location_id'],$pdo);
    
    echo'<td><input type="number" class="form-control stock" name="stock[]" value="'.$stock_db.'" readonly></td>';
    ...
    echo'<td><center><button type="button" name="remove" class="btn btn-danger btn-sm btnremove" role="button"><span class="glyphicon glyphicon-remove"></span></button><center></td></center>';
    ...
    <!-- radio -->
              ...
              <div class="form-group">
                ...
                <label style="margin: 0 15px;">
                  <input type="radio" name="rb" class="minimal-red" value="CASH"<?php echo ($payment_type=='CASH')?'checked':''?> checked>CASH
                </label>
                <label style="margin: 0 15px;">
                  <input type="radio" name="rb" class="minimal-red" value="MPESA" <?php echo ($payment_type=='MPESA')?'checked':''?>>MPESA
                </label>
                <label style="margin: 0 15px;">
                  <input type="radio" name="rb" class="minimal-red" value="CHEQUE" <?php echo ($payment_type=='CHEQUE')?'checked':''?>>CHEQUE
                </label>
                <label style="margin: 0 15px;">
                  <input type="radio" name="rb" class="minimal-red" value="EQUITEL" <?php echo ($payment_type=='EQUITEL')?'checked':''?>>EQUITEL
                </label>
                <label style="margin: 0 15px;">
                  <input type="radio" name="rb" class="minimal-red" value="CREDIT" <?php echo ($payment_type=='CREDIT')?'checked':''?>>CREDIT
                </label>
              </div>
        ...
        <div class="col-md-5" style="display: flex; padding-top: 20px; justify-content: flex-end;  ">
            ...
            <div align="center" style="width: 100%; margin-left: 20px;">
                 <input type="submit" name="btnupdateorder" value="Update Order" class="btn btn-warning col-md-12">
            </div>               
         </div>  
...
$(document).ready(function(){
    ...
    var NetTotal = $("#txttotal").val();

    console.log("Subtotal: "+SubTotal);
    console.log("NetTotal: "+NetTotal);

    if (SubTotal == NetTotal) {
      $('#exclusive_tax').prop('checked',false);
    }else{
      $('#exclusive_tax').prop('checked',true);
    }
