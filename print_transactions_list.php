...
class ConductFPDF extends FPDF {
	...
	$pdf->SetFont('Arial','B','16');
	$pdf->SetX(80);
	$pdf->Cell(130,10,'#MPESA TRANSACTIONS LIST',0,1,'C');
	...
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("Transaction Time"));
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("Code"));
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("Amount"));
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("First Name"));
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("Last Name"));
	$x_axis = $pdf->getx();
	$pdf->vcell(45,10,$x_axis,strtoupper("Phone"));
	...
	while($row=$select->fetch(PDO::FETCH_OBJ)){
		...
		$x_axis = $pdf->getx();
		$pdf->vcell(10,10,$x_axis,strtoupper($row->id));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->TransTime));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->TransID));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->TransAmount));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->FirstName));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->LastName));
		$x_axis = $pdf->getx();
		$pdf->vcell(45,10,$x_axis,strtoupper($row->MSISDN));
